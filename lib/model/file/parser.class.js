...
const {
  ...
  hasFailScriptRegex,
  nextConditionRegex
} = require("@editide/awesome-js-regex");
class Parser {
  constructor({
    lines,
    currentLineIndex = 0,
  ...
  parseContext(isCondition) {
    ...
    do {
      this.nextLine();
      if (!this.currentLine || isCondition && this.currentLine.match(closeBracket) && (this.currentLine.match(nextConditionRegex) || this.currentLine.match(hasFailScriptRegex))) {
        break;
      }
